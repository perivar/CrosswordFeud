...
const getCurrentItems = (items: any[], currentPage: number, itemsPerPage: number) => {
...
const usePagination = (props: IPaginationProps) => {
  const initialState = React.useMemo(() => getInitialState(props), [props]);

  const [state, dispatch] = React.useReducer(reducer, initialState);

  const onNextPage = React.useCallback(() => dispatch({ type: 'next' }), []);

  const onPreviousPage = React.useCallback(() => dispatch({ type: 'previous' }), []);

  const onResetPage = React.useCallback(() => dispatch({ type: 'reset', initialState: props }), [props]);

  const setCurrentPage = React.useCallback(currentPage => dispatch({ type: 'set', currentPage }), []);
