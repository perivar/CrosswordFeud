...
function get() {
    // action creators
    function request(): CrosswordActions { return { type: CrosswordActionTypes.GET_REQUEST } }
    function success(data: any): CrosswordActions { return { type: CrosswordActionTypes.GET_SUCCESS, data: data } }
    function failure(error: string): CrosswordActions { return { type: CrosswordActionTypes.GET_FAILURE, error } }

    return (dispatch: any) => {
        dispatch(request());

        crosswordService.get()
            .then(
                (data: any) => {
